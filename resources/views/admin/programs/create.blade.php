@extends('layouts.app')
@section('title', 'Nuevo Programa de Estudio - Panel Administrativo')
@section('content')
<div id="dashboard-container" class="flex w-full bg-gray-50 font-sans text-gray-900 min-h-[calc(100vh-64px)]" x-data="enterpriseApp()">
    ...
    <div class="flex-1 flex flex-col min-w-0 bg-gray-50/50 relative">  
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8 overflow-x-hidden" x-data="{ logoPreview: null }">
                    ...
                    <form action="{{ route('admin.programs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="5" required
                                    ...
                                    placeholder="Describe las competencias, perfil del egresado y objetivos del programa...">{{ old('description') }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="details" name="details" rows="3" required
                                    ...
                                    placeholder="Ej. Duración: 3 años (06 períodos lectivos)&#10;Título: Profesional Técnico en Producción Agropecuaria">{{ old('details') }}</textarea>
                                @error('details')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-gray-400 mt-1.5">Formatos permitidos: PNG, JPG, JPEG, WEBP. Máx. 2MB.</p>
                                @error('logo_path')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                            ...
                            <button type="submit" class="px-5 py-2.5 text-xs font-bold text-white bg-purple-600 rounded-xl hover:bg-purple-700 transition flex items-center gap-1.5 shadow-sm">
                                <i class="bi bi-floppy"></i> Guardar Programa
                            </button>
                        </div>
                    </form>
                </div>
