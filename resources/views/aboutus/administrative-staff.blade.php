...
@section('content')
    <div x-data="{
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                    <div class="lg:col-span-8">
                        <h1
                            class="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight font-sans leading-tight">
                        ...
                        <p class="mt-4 text-base sm:text-lg text-slate-300 max-w-2xl font-normal leading-relaxed">
                            Conoce al personal directivo, administrativo y de servicios que impulsa la gestión
                            institucional, garantizando una atención eficiente, transparente y de excelencia a nuestra
                            comunidad educativa.
                        </p>
                    </div>
                    ...
                    <div class="lg:col-span-4 grid grid-cols-2 gap-4">
                        <div
                            class="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-4 text-center hover:bg-white/10 transition-colors">
                        ...
                        <div
                            class="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-4 text-center hover:bg-white/10 transition-colors">
                            <span
                                class="block text-3xl font-extrabold text-emerald-400 font-display">{{ count($staffs) }}</span>
        ...
        <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-7 relative z-20">
            <div
                class="bg-white rounded-2xl shadow-xl shadow-slate-200/60 p-2 border border-slate-200/80 flex flex-wrap sm:flex-nowrap gap-2">
                <button @click="activeTab = 'directory'"
                    :class="activeTab === 'directory' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' :
                ...
                <button @click="activeTab = 'organigram'"
                    :class="activeTab === 'organigram' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' :
                        'text-slate-600 hover:bg-slate-100 hover:text-slate-900'"
                    class="flex-1 py-3 px-4 rounded-xl font-semibold text-sm transition-all duration-200 flex items-center justify-center gap-2">
                ...
                <button @click="activeTab = 'cargos'"
                    :class="activeTab === 'cargos' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' :
                        'text-slate-600 hover:bg-slate-100 hover:text-slate-900'"
                    class="flex-1 py-3 px-4 rounded-xl font-semibold text-sm transition-all duration-200 flex items-center justify-center gap-2">
        ...
        <div x-show="activeTab === 'directory'"
            ...
            <div
                class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm mb-8 flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div class="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-1 md:pb-0">
                    <span
                        class="text-xs font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap mr-1">Filtrar:</span>
                    <button @click="levelFilter = 'all'"
                        :class="levelFilter === 'all' ? 'bg-slate-900 text-white font-semibold' :
                            'bg-slate-100 text-slate-600 hover:bg-slate-200'"
                        class="px-3 py-1.5 rounded-lg text-xs transition-colors whitespace-nowrap">
                        Todos ({{ count($staffs) }})
                    </button>
                    <button @click="levelFilter = 'directivos'"
                        :class="levelFilter === 'directivos' ? 'bg-blue-600 text-white font-semibold' :
                            'bg-slate-100 text-slate-600 hover:bg-slate-200'"
                        class="px-3 py-1.5 rounded-lg text-xs transition-colors whitespace-nowrap">
                        Dirección & Jefaturas
                    </button>
                    <button @click="levelFilter = 'apoyo'"
                        :class="levelFilter === 'apoyo' ? 'bg-amber-600 text-white font-semibold' :
                            'bg-slate-100 text-slate-600 hover:bg-slate-200'"
                        class="px-3 py-1.5 rounded-lg text-xs transition-colors whitespace-nowrap">
                        Asistentes & Apoyo
                    </button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($staffs as $staff)
                    @php
                        ...
                        $isSupport =
                            str_contains($job, 'asistente') ||
                            str_contains($job, 'auxiliar') ||
                            str_contains($job, 'seguridad') ||
                            str_contains($job, 'servicio');
                    ...
                    <div x-show="(levelFilter === 'all' || levelFilter === '{{ $category }}') && ('{{ strtolower($staff->names) }}'.includes(searchQuery.toLowerCase()) || '{{ strtolower($staff->job_position ?? '') }}'.includes(searchQuery.toLowerCase()) || '{{ strtolower($staff->email ?? '') }}'.includes(searchQuery.toLowerCase()))"
                        ...
                        <div class="p-6">
                            <div class="flex items-start justify-between gap-4 mb-4">
                                ...
                                @if ($isDirector)
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-amber-100 text-amber-800 border border-amber-200">
                                ...
                                @elseif($isSecretary)
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-purple-100 text-purple-800 border border-purple-200">
                                        <i class="bi bi-file-earmark-text mr-1"></i> Secretaría
                                    </span>
                                @else
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-slate-100 text-slate-700 border border-slate-200">
                        ...
                        <div class="bg-slate-50 px-6 py-3 border-t border-slate-100 flex items-center justify-between">
                            <span class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">IESTP
                                FVC</span>
                ...
                @empty
                    <div class="col-span-full bg-white rounded-2xl p-12 text-center border border-slate-200 shadow-sm">
                        ...
                        <p class="text-sm text-slate-500 mt-1">Actualmente no hay personal administrativo asignado que
                            coincida con los criterios.</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div x-show="activeTab === 'organigram'"
            x-transition:enter="transition ease-out duration-300 transform opacity-0 translate-y-2"
            class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
            ...
            <div class="bg-white rounded-3xl border border-slate-200/90 shadow-sm p-6 sm:p-8">
                <div
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-slate-900 font-sans">Estructura Organizacional Institucional
                        </h2>
                        <p class="text-sm text-slate-600 mt-1">Niveles jerárquicos, áreas de gestión y dependencias
                            operativas del IESTP Francisco Vigo Caballero.</p>
                    </div>
                    <div class="flex items-center gap-3 text-xs font-semibold">
                        <span
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-50 text-amber-900 border border-amber-200">
                            <span class="w-2.5 h-2.5 rounded-full bg-amber-500"></span> Nivel 1: Alta Dirección
                        </span>
                        <span
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 text-blue-900 border border-blue-200">
                            <span class="w-2.5 h-2.5 rounded-full bg-blue-500"></span> Nivel 2: Gestión & Unidades
                        </span>
                        <span
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-900 border border-emerald-200">
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-amber-500/10 border-2 border-amber-300/60 rounded-2xl p-6 relative">
                        <div
                            class="absolute -top-3.5 left-6 bg-amber-500 text-white font-extrabold text-xs px-3 py-1 rounded-full uppercase tracking-wider shadow-sm">
                        ...
                        <div class="pt-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="org-box-solid p-4 text-center rounded-xl bg-white border-2 border-amber-600">
                                <span class="text-xs font-extrabold text-amber-700 uppercase tracking-wide block">Dirección
                                    General</span>
                            ...
                            <div class="org-box-dashed p-4 text-center rounded-xl bg-white">
                                <span class="text-xs font-extrabold text-slate-700 uppercase tracking-wide block">Consejo
                                    Asesor</span>
                                <span class="text-xs text-slate-500 mt-1 block">Órgano Consultivo e Institucional</span>
                            </div>
                            <div class="org-box-dashed p-4 text-center rounded-xl bg-white">
                                <span class="text-xs font-extrabold text-slate-700 uppercase tracking-wide block">Concejo
                                    Estudiantil</span>
                    ...
                    <div class="bg-blue-500/10 border-2 border-blue-300/60 rounded-2xl p-6 relative">
                        <div
                            class="absolute -top-3.5 left-6 bg-blue-600 text-white font-extrabold text-xs px-3 py-1 rounded-full uppercase tracking-wider shadow-sm">
                        ...
                        <div class="pt-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            ...
                            <div class="org-box-solid p-3.5 rounded-xl bg-white">
                                <span class="text-xs font-bold text-blue-900 block">Unidad de Bienestar &
                                    Empleabilidad</span>
                        ...
                        <div class="mt-6 pt-6 border-t border-blue-200/80">
                            <span
                                class="text-xs font-extrabold text-blue-800 uppercase tracking-wider block mb-3">Coordinaciones
                                de Programas de Estudio</span>
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                                <div class="p-3 bg-white rounded-xl border border-slate-200 text-center">
                                    <span class="text-[11px] font-bold text-slate-800 block">Coord. Producción
                                        Agropecuaria</span>
                                </div>
                                <div class="p-3 bg-white rounded-xl border border-slate-200 text-center">
                                    <span class="text-[11px] font-bold text-slate-800 block">Coord. Enfermería
                                        Técnica</span>
                                </div>
                                <div class="p-3 bg-white rounded-xl border border-slate-200 text-center">
                                    <span class="text-[11px] font-bold text-slate-800 block">Coord. Adm. Redes y
                                        Com.</span>
                                </div>
                                <div class="p-3 bg-white rounded-xl border border-slate-200 text-center">
                                    <span class="text-[11px] font-bold text-slate-800 block">Coord. Asistencia
                                        Administrativa</span>
                                </div>
                                <div class="p-3 bg-white rounded-xl border border-slate-200 text-center">
                                    <span class="text-[11px] font-bold text-slate-800 block">Coord. Manejo Forestal /
                                        RN</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-emerald-500/10 border-2 border-emerald-300/60 rounded-2xl p-6 relative">
                        <div
                            class="absolute -top-3.5 left-6 bg-emerald-600 text-white font-extrabold text-xs px-3 py-1 rounded-full uppercase tracking-wider shadow-sm">
                        ...
                        <div class="pt-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                                <h4
                                    class="text-xs font-extrabold text-emerald-800 uppercase tracking-wide border-b pb-2 mb-3">
                                    Administración & Logística</h4>
                                <ul class="space-y-2 text-xs text-slate-700">
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Asistente Administrativo</li>
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Tesorería y Caja</li>
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Área de Patrimonio</li>
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Área de Abastecimiento</li>
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Guardianía Diurna / Nocturna</li>
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Personal de Campo y Servicios</li>
                                </ul>
                            </div>
                            ...
                            <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                                <h4
                                    class="text-xs font-extrabold text-emerald-800 uppercase tracking-wide border-b pb-2 mb-3">
                                    Cuerpo Docente & Estudiantes</h4>
                                <ul class="space-y-2 text-xs text-slate-700">
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Docentes de Módulos Profesionales</li>
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Docentes Transversales (Empleabilidad)</li>
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Estudiantes de Educación Superior</li>
                                </ul>
                            </div>
                            ...
                            <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                                <h4
                                    class="text-xs font-extrabold text-emerald-800 uppercase tracking-wide border-b pb-2 mb-3">
                                    Servicios de Bienestar & Salud</h4>
                                <ul class="space-y-2 text-xs text-slate-700">
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Servicio Médico (Tópico Institucional)</li>
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Servicio Psicopedagógico</li>
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Bienestar Social y Consejería</li>
                                    <li class="flex items-center gap-2"><i class="bi bi-dot text-emerald-600 text-lg"></i>
                                        Inserción Laboral y Empleabilidad</li>
                                </ul>
                            </div>
                        </div>
                    </div>
        ...
        <div x-show="activeTab === 'cargos'"
            ...
            <div class="bg-white rounded-3xl border border-slate-200/90 shadow-sm p-6 sm:p-8">
                <div
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-slate-900 font-sans">Cuadro Institucional de Cargos y Personal
                        </h2>
                        <p class="text-sm text-slate-600 mt-1">Resumen detallado de plazas institucionales, condición
                            laboral y asignación por programa de estudios.</p>
                    </div>
                    <div
                        class="px-4 py-2 bg-blue-50 border border-blue-200 rounded-xl text-blue-900 text-xs font-bold flex items-center gap-2">
                    ...
                    <table class="w-full text-left text-xs text-slate-700">
                        ...
                        <tfoot class="bg-slate-900 text-white font-bold">
                            <tr>
                                ...
                                <td class="py-3.5 px-3 font-normal text-xs text-slate-300" colspan="2">Consolidado
                                    Institucional de Personal</td>
                ...
                <div class="mt-4 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500">
                    <p><strong>Leyenda:</strong> P.A. (Producción Agropecuaria), E.T. (Enfermería Técnica), A.R. y C. (Adm.
                        de Redes y Com.), A.A. (Asistencia Adm.), MA y RN (Medio Ambiente y Recursos Naturales).</p>
                    <span class="font-semibold text-slate-600">Fuente: Cuadro PAP Aprobado — IESTP Francisco Vigo
                        Caballero</span>
                </div>
            </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="modalOpen" x-transition:enter="ease-out duration-300"
                    ...
                    <button @click="closeDetail()"
                        class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center transition-colors">
                    ...
                    <template x-if="selectedStaff">
                        <div>
                            <div class="flex items-center gap-4 mb-6">
                                ...
                                <div>
                                    <h3 class="text-xl font-bold text-slate-900 font-sans" x-text="selectedStaff.names">
                                    </h3>
                                    <p class="text-sm font-semibold text-blue-600"
                                        x-text="selectedStaff.job_position || 'Personal Administrativo'"></p>
                                    <span
                                        class="inline-block px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-100 text-emerald-800 mt-1">
                            ...
                            <div class="mt-6 pt-4 border-t border-slate-100 flex justify-end">
                                <button @click="closeDetail()"
                                    class="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs rounded-xl transition-colors">
                                    Cerrar Ficha
                                </button>
                            </div>
                        </div>
