@extends('layouts.app')
@section('title', 'Editar Módulo de Certificación - Panel Administrativo')
@section('content')
            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <h1 class="text-xl sm:text-2xl font-extrabold text-gray-800 tracking-tight">
                        Editar Módulo de Certificación
                    </h1>
                </div>
                ...
                <div class="hidden sm:flex items-center text-sm font-medium text-gray-500">
                    ...
                    <span class="text-purple-600">Editar Módulo</span>
                </div>
            </div>
        </header>
            ...
            <div class="max-w-3xl mx-auto space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 space-y-8">
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r from-purple-500/10 via-indigo-500/10 to-blue-500/10 border border-purple-100">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-gray-900">Modificar Módulo: {{ $module->module }}</h3>
                            <p class="text-xs text-gray-500 mt-0.5">Actualiza el nombre, programa asignado o estado del módulo.</p>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('admin.programs.modules.update', $module) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <select id="program_id" name="program_id" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('program_id') border-red-500 @enderror">
                                    ...
                                    @foreach($programs as $prog)
                                        <option value="{{ $prog->id }}" {{ old('program_id', $module->program_id) == $prog->id ? 'selected' : '' }}>
                                            {{ $prog->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="module" name="module" value="{{ old('module', $module->module) }}" required class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('module') border-red-500 @enderror">
                                </div>
                                @error('module')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                                @enderror
