    ...
    <div class="flex flex-col h-full lg:h-[calc(100vh-64px)] lg:sticky lg:top-[64px] overflow-hidden">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto sidebar-scrollbar custom-scrollbar">
            ...
            <a href="{{ route('admin.library.index') }}"
                class="flex items-center px-4 py-3.5 rounded-xl transition-all duration-200 group relative overflow-hidden {{ request()->routeIs('admin.library.*') ? 'bg-gradient-to-r from-purple-600/20 to-indigo-600/20 text-white shadow-lg shadow-purple-900/20' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-100' }}">
                <div
                    class="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-purple-500 to-indigo-600 rounded-r-md transition-all duration-200 {{ request()->routeIs('admin.library.*') ? 'scale-y-100' : 'scale-y-0 group-hover:scale-y-75' }}">
                </div>
                <i
                    class="bi bi-book-half text-xl transition-transform duration-200 group-hover:scale-110 {{ request()->routeIs('admin.library.*') ? 'text-purple-400' : 'text-slate-400 group-hover:text-purple-400' }}"></i>
                <span class="ml-4 font-medium whitespace-nowrap" x-show="sidebarOpen">Biblioteca Virtual</span>
                @if (request()->routeIs('admin.library.*'))
                    <div
                        class="absolute right-4 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-purple-400 shadow-lg shadow-purple-400/50">
                    </div>
                @endif
            ...
            <!-- ── GRUPO: Transparencia ──────────────────────────────────── -->
            @php $transparencyOpen = request()->routeIs('admin.claims.*', 'admin.statistics.*', 'admin.degree-records.*', 'admin.licensing.*', 'admin.account-balances.*'); @endphp
            <div x-data="{ open: {{ $transparencyOpen ? 'true' : 'false' }} }">
                ...
                <div x-show="open && sidebarOpen" x-transition:enter="transition ease-out duration-200"
                    ...
                    <a href="{{ route('admin.degree-records.index') }}"
                        class="flex items-center px-3 py-2.5 rounded-xl transition-all duration-200 group relative overflow-hidden
                            {{ request()->routeIs('admin.degree-records.*') ? 'bg-purple-600/15 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-100' }}">
                        <i
                            class="bi bi-award text-base group-hover:scale-110 transition-transform duration-200
                    ...
                    <a href="{{ route('admin.account-balances.index') }}"
                        class="flex items-center px-3 py-2.5 rounded-xl transition-all duration-200 group relative overflow-hidden
                            {{ request()->routeIs('admin.account-balances.*') ? 'bg-purple-600/15 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-100' }}">
                        <i
                            class="bi bi-cash-stack text-base group-hover:scale-110 transition-transform duration-200
                                {{ request()->routeIs('admin.account-balances.*') ? 'text-purple-400' : 'text-slate-500 group-hover:text-purple-400' }}"></i>
                        <span class="ml-3 text-sm font-medium whitespace-nowrap">Inversión y Gastos</span>
                        @if (request()->routeIs('admin.account-balances.*'))
                            <div
                                class="absolute right-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-purple-400 shadow-lg shadow-purple-400/50">
                            </div>
                        @endif
