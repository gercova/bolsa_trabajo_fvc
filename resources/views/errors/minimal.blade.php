...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700&family=DM+Serif+Display:ital@0;1&display=swap"
        rel="stylesheet">
...
<body
    class="h-full text-slate-100 flex items-center justify-center p-4 relative antialiased selection:bg-gold selection:text-white">
    ...
    <div class="w-full max-w-xl glass-card rounded-3xl p-8 md:p-12 text-center relative z-10 fade-in-up">
        ...
        <div class="flex items-center justify-center gap-3 mb-8">
            <div
                class="w-11 h-11 bg-slate-900 border border-slate-700/50 rounded-xl flex items-center justify-center shadow-lg">
            ...
            <div class="text-left leading-none">
                ...
                <h2 class="text-sm font-serif text-slate-200 mt-0.5">IESTP <span
                        class="italic text-gold-vivid">Francisco Vigo Caballero</span></h2>
            </div>
        </div>

        <!-- Error Icon with pulse/glow -->
        <div
            class="inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-gold-vivid/10 border border-gold-vivid/20 text-gold-vivid mb-6">
            <i class="bi @yield('icon', 'bi-exclamation-triangle-fill') text-4xl"></i>
        </div>

        <!-- Error Code -->
        <h1
            class="text-7xl md:text-8xl font-black tracking-tighter bg-gradient-to-b from-slate-100 via-slate-200 to-slate-400 bg-clip-text text-transparent mb-2">
            @yield('code')
        </h1>
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <button onclick="window.history.back()"
                class="w-full sm:w-auto px-6 py-3 rounded-xl border border-slate-700 hover:border-slate-500 bg-slate-800/40 hover:bg-slate-800 text-slate-300 font-semibold text-sm transition-all duration-200 flex items-center justify-center gap-2">
                <i class="bi bi-arrow-left"></i> Regresar
            </button>
            <a href="{{ route('inicio') }}"
                class="w-full sm:w-auto px-7 py-3 rounded-xl bg-gold-vivid hover:bg-amber-600 text-white font-semibold text-sm shadow-lg shadow-gold-vivid/20 hover:shadow-gold-vivid/30 transition-all duration-200 hover:-translate-y-0.5 flex items-center justify-center gap-2">
