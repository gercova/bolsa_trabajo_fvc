@extends('layouts.app')
@section('title', 'Editar Requisito de Admisión - Panel Administrativo')
@section('content')
            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <h1 class="text-xl sm:text-2xl font-extrabold text-gray-800 tracking-tight">
                        Editar Requisito del Programa
                    </h1>
                </div>

                <div class="hidden sm:flex items-center text-sm font-medium text-gray-500">
                    <a href="{{ route('admin.programs.index', ['tab' => 'requirements']) }}" class="hover:text-purple-600">Requisitos</a>
                    <i class="bi bi-chevron-right mx-2 text-xs text-gray-400"></i>
                    <span class="text-purple-600">Editar Requisito</span>
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8 overflow-x-hidden">
            <div class="max-w-3xl mx-auto space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <a href="{{ route('admin.programs.index', ['tab' => 'requirements']) }}" class="inline-flex items-center gap-2 text-sm font-semibold text-gray-600 hover:text-purple-600 transition-colors">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 space-y-8">
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r from-purple-500/10 via-indigo-500/10 to-blue-500/10 border border-purple-100">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-gray-900">Modificar Requisito</h3>
                            <p class="text-xs text-gray-500 mt-0.5">Actualiza la descripción del requisito, programa asignado u orden.</p>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('admin.programs.requirements.update', $requirement) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <select id="study_program_id" name="study_program_id" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('study_program_id') border-red-500 @enderror">
                                    ...
                                    @foreach($programs as $prog)
                                        <option value="{{ $prog->id }}" {{ old('study_program_id', $requirement->study_program_id) == $prog->id ? 'selected' : '' }}>
                                            {{ $prog->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <textarea id="description" name="description" rows="3" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('description') border-red-500 @enderror">{{ old('description', $requirement->description) }}</textarea>
                                @error('description')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <input type="number" id="order" name="order" value="{{ old('order', $requirement->order) }}" min="0" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">
                            </div>
                            ...
                            <div class="pt-2">
                                <label class="inline-flex items-center cursor-pointer gap-3 p-3 bg-gray-50 border border-gray-200 rounded-xl hover:bg-gray-100/80 transition-colors w-full">
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $requirement->is_active) ? 'checked' : '' }} class="sr-only peer">
                                    ...
                                    <div>
                                        <span class="text-sm font-bold text-gray-900 block">Requisito Activo</span>
                                        <span class="text-xs text-gray-500 block">Los requisitos activos se visualizan públicamente en la ficha de admisión del programa.</span>
                                    </div>
                                </label>
                            </div>
                        ...
                        <div class="pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
                            <a href="{{ route('admin.programs.index', ['tab' => 'requirements']) }}" class="px-5 py-2.5 bg-gray-100 text-gray-700 font-semibold text-sm rounded-xl hover:bg-gray-200 transition-colors">
                                Cancelar
                            </a>
                            <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold text-sm rounded-xl shadow-md hover:from-purple-700 hover:to-indigo-700 transition-all duration-200 inline-flex items-center gap-2">
                                ...
                                <span>Actualizar Requisito</span>
                            </button>
                        </div>
                    </form>
