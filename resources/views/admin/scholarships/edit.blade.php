@extends('layouts.app')
@section('title', 'Editar Modalidad de Beca - Panel Administrativo')
            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <h1 class="text-xl sm:text-2xl font-extrabold text-gray-800 tracking-tight flex items-center gap-2">
                        <i class="bi bi-pencil-square text-purple-600"></i> Editar Modalidad: {{ $scholarship->name }}
                    </h1>
                </div>
                ...
                <div class="hidden sm:flex items-center text-sm font-medium text-gray-500">
                    ...
                    <span class="text-purple-600">Editar Beca</span>
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8 overflow-x-hidden">
            <div class="max-w-4xl mx-auto space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    <form action="{{ route('admin.scholarships.update', $scholarship) }}" method="POST" class="p-6 sm:p-8 space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="slug" id="slug" value="{{ old('slug', $scholarship->slug) }}" 
                            ...
                            <div>
                                ...
                                <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $scholarship->sort_order) }}" min="0"
                            ...
                            <div>
                                <label for="vacancies" class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-2">
                                    <i class="bi bi-people-fill text-purple-600 mr-1"></i> Plazas / Vacantes Disponibles
                                </label>
                                <input type="number" name="vacancies" id="vacancies" value="{{ old('vacancies', $scholarship->vacancies ?? 0) }}" min="0"
                                    placeholder="Ej. 10"
                                    class="w-full text-sm font-semibold px-4 py-2.5 border @error('vacancies') border-rose-500 @else border-gray-300 @enderror rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all">
                                <p class="text-[11px] text-gray-500 mt-1">Número de vacantes preferenciales reservadas para esta modalidad.</p>
                                @error('vacancies')
                                    <p class="text-xs text-rose-600 mt-1 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                <label for="discount_percentage" class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-2">
                                    <i class="bi bi-percent text-purple-600 mr-1"></i> Porcentaje de Descuento (%)
                                </label>
                                <div class="relative">
                                    <input type="number" step="0.01" min="0" max="100" name="discount_percentage" id="discount_percentage" 
                                        value="{{ old('discount_percentage', $scholarship->discount_percentage ?? 0) }}" placeholder="Ej. 100.00 o 50.00"
                                        class="w-full text-sm font-semibold px-4 py-2.5 pr-8 border @error('discount_percentage') border-rose-500 @else border-gray-300 @enderror rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all">
                                    <span class="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-xs">%</span>
                                </div>
                                <p class="text-[11px] text-gray-500 mt-1">Beneficio principal (100% para exoneración total, 50% para media beca, etc.).</p>
                            ...
                            <div class="md:col-span-2">
                                <label for="discount_details" class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-2">
                                    <i class="bi bi-tags-fill text-purple-600 mr-1"></i> Detalle / Desglose de Descuentos
                                </label>
                                <input type="text" name="discount_details" id="discount_details" value="{{ old('discount_details', $scholarship->discount_details) }}" 
                                    placeholder="Ej. 100% descuento 1° puesto y 50% descuento 2° puesto / 50% descuento Servicio Militar"
                                    class="w-full text-sm px-4 py-2.5 border @error('discount_details') border-rose-500 @else border-gray-300 @enderror rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all">
                                <p class="text-[11px] text-gray-500 mt-1">Explicación textual visible en el portal para el postulante.</p>
                            ...
                            {{-- Icono Bootstrap --}}
                            <div class="md:col-span-2 space-y-3" x-data="{ selectedIcon: '{{ old('icon', $scholarship->icon ?? 'bi-award') }}' }">
                            ...
                            <div class="md:col-span-2">
                                <label class="inline-flex items-center gap-3 cursor-pointer">
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $scholarship->is_active) ? 'checked' : '' }}
                            ...
                            <div class="md:col-span-2">
                                <label for="description" class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-2">
                                    Descripción Completa de la Modalidad
                                </label>
                                <textarea name="description" id="description" rows="4" 
                                    placeholder="Ingrese los detalles de la beca, público objetivo y alcance de los beneficios..."
                                    class="w-full text-sm p-4 border @error('description') border-rose-500 @else border-gray-300 @enderror rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all leading-relaxed">{{ old('description', $scholarship->description) }}</textarea>
                                @error('description')
                                    <p class="text-xs text-rose-600 mt-1 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2">
                                <label for="requirements" class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-2">
                                    <i class="bi bi-file-earmark-check-fill text-purple-600 mr-1"></i> Requisitos y Documentación Solicitada
                                </label>
                                <textarea name="requirements" id="requirements" rows="4" 
                                    placeholder="Ingrese los documentos exigidos (uno por línea)..."
                                    class="w-full text-sm p-4 border @error('requirements') border-rose-500 @else border-gray-300 @enderror rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all leading-relaxed">{{ old('requirements', $scholarship->requirements) }}</textarea>
                                <p class="text-[11px] text-gray-500 mt-1">Opcional. Se mostrará en la ficha modal detallada de la beca.</p>
                                @error('requirements')
                                    <p class="text-xs text-rose-600 mt-1 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="pt-6 border-t border-gray-200 flex items-center justify-end gap-3">
                            ...
                            <button type="submit" 
                                ...
                                <i class="bi bi-save"></i> Actualizar Beca
                            </button>
                        </div>
