@extends('layouts.app')
@section('title', 'Gestión de Itinerarios — Panel Administrativo')
...
<div id="dashboard-container" class="flex w-full bg-gray-50 font-sans text-gray-900 min-h-[calc(100vh-64px)]"
    ...
    <div class="flex-1 flex flex-col min-w-0 bg-gray-50/50 relative" x-data="itinerariesApp()">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-xl sm:text-2xl font-extrabold text-gray-800 tracking-tight leading-none flex items-center gap-2">
                            <i class="bi bi-diagram-3 text-purple-600"></i> Itinerarios Formativos
                        </h1>
                        <p class="text-xs text-gray-400 font-medium mt-0.5">Gestión de itinerarios, temas y secuencias formativas por módulo y curso</p>
                    </div>
                </div>
                <div class="hidden sm:flex items-center text-sm font-medium text-gray-500">
                    <i class="bi bi-book mr-1 text-purple-500"></i> Programas
                    <i class="bi bi-chevron-right mx-2 text-xs text-gray-400"></i>
                    <span class="text-purple-600">Itinerarios</span>
                </div>
            </div>
            ...
            <div class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div class="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Total Itinerarios</p>
                            <h3 class="text-2xl font-black text-gray-800">{{ number_format($totalItineraries) }}</h3>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Itinerarios Activos</p>
                            <h3 class="text-2xl font-black text-emerald-700">{{ number_format($activeItineraries) }}</h3>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-5 rounded-2xl border border-gray-200 shadow-xs flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">Cursos con Itinerario</p>
                            <h3 class="text-2xl font-black text-blue-700">{{ number_format($coursesWithItin) }}</h3>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 space-y-4">
                    <form action="{{ route('admin.itineraries.index') }}" method="GET"
                        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-center">

                        {{-- Search --}}
                        <div class="lg:col-span-4 relative">
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="Buscar por nombre, módulo o curso..."
                        ...
                        <div class="lg:col-span-3 flex items-center gap-2 justify-end">
                            ...
                            @if (request()->hasAny(['search', 'course_id', 'module_id', 'status']))
                                <a href="{{ route('admin.itineraries.index') }}"
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-gray-100">
                        <span class="text-xs font-semibold text-gray-500">
                            Mostrando {{ $itineraries->total() }} itinerarios registrados
                        </span>
                        <button type="button" @click="openCreateModal()"
                            ...
                            <i class="bi bi-plus-lg"></i> Nuevo Itinerario
                        </button>
                    </div>
                </div>
                    ...
                    @if ($itineraries->isEmpty())
                        <div class="flex flex-col items-center justify-center py-20 gap-4 text-gray-400">
                            ...
                            <div class="text-center">
                                <p class="font-bold text-gray-700">Sin itinerarios registrados</p>
                                <p class="text-sm text-gray-500 mt-1">
                                    @if (request()->hasAny(['search', 'course_id', 'module_id', 'status']))
                                        No hay itinerarios con los filtros aplicados.
                                        <a href="{{ route('admin.itineraries.index') }}" class="text-purple-600 font-semibold hover:underline">Limpiar filtros</a>
                                    @else
                                        Empieza registrando el primer itinerario con el botón <strong>Nuevo Itinerario</strong>.
                                    @endif
                                </p>
                            </div>
                        </div>
                            ...
                            <table class="min-w-full text-sm">
                                <thead class="bg-gray-50 border-b border-gray-200">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Módulo</th>
                                        <th class="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Itinerario / Contenido</th>
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <div class="flex items-center gap-2">
                                                    <span class="px-2 py-0.5 rounded-md font-mono text-xs font-bold bg-purple-100 text-purple-700">
                                                        {{ $itin->course?->code ?? 'N/A' }}
                                                    </span>
                                                    <span class="font-semibold text-gray-800 text-xs truncate max-w-[180px]" title="{{ $itin->course?->name }}">
                                                        {{ $itin->course?->name ?? '—' }}
                                                    </span>
                                                </div>
                                            </td>
                                            <td class="px-4 py-3">
                                                <div class="font-semibold text-gray-700 text-xs truncate max-w-[200px]" title="{{ $itin->module?->name }}">
                                                    {{ $itin->module?->name ?? '—' }}
                                                </div>
                                            </td>
                                            <td class="px-4 py-3">
                                                <div class="font-bold text-gray-800 text-sm">{{ $itin->name }}</div>
                                            </td>
                                            <td class="px-4 py-3 text-center whitespace-nowrap">
                                                <form action="{{ route('admin.itineraries.toggle-status', $itin) }}" method="POST" class="inline-block">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
                                                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold transition {{ $itin->is_active ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                                                        <span class="w-1.5 h-1.5 rounded-full {{ $itin->is_active ? 'bg-emerald-500' : 'bg-gray-400' }}"></span>
                                                        {{ $itin->is_active ? 'Activo' : 'Inactivo' }}
                                                    </button>
                                                </form>
                                            </td>
                                            <td class="px-4 py-3 text-center whitespace-nowrap">
                                                <div class="flex items-center justify-center gap-1.5">
                                                    <button type="button" @click="openEditModal({{ json_encode($itin) }})"
                                                        class="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
                                                        title="Editar Itinerario">
                                                        <i class="bi bi-pencil-square text-base"></i>
                                                    </button>
                                                    <form action="{{ route('admin.itineraries.destroy', $itin) }}" method="POST"
                                                        onsubmit="return confirm('¿Está seguro de eliminar el itinerario «{{ addslashes($itin->name) }}»?')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                        ...
                        @if ($itineraries->hasPages())
                            <div class="px-6 py-4 border-t border-gray-100 flex flex-wrap items-center justify-between gap-3">
                                <p class="text-xs text-gray-500">
                                    Mostrando <strong>{{ $itineraries->firstItem() }}</strong>–<strong>{{ $itineraries->lastItem() }}</strong> de <strong>{{ $itineraries->total() }}</strong> itinerarios
                                </p>
                                {{ $itineraries->links() }}
                            </div>
                        @endif
        ...
        <div x-show="modalOpen" x-cloak
            ...
            <div @click.outside="modalOpen = false"
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-purple-50 to-indigo-50">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-purple-100 text-purple-700 flex items-center justify-center text-xl">
                            <i :class="isEdit ? 'bi-pencil-square' : 'bi-diagram-3-fill'"></i>
                        </div>
                        <div>
                            <h3 class="text-base font-extrabold text-gray-900" x-text="isEdit ? 'Editar Itinerario' : 'Nuevo Itinerario'"></h3>
                            <p class="text-xs text-gray-500">Asigne el itinerario al curso y módulo correspondiente</p>
                        </div>
                    </div>
                ...
                {{-- Modal Form --}}
                <form :action="isEdit ? updateUrl : '{{ route('admin.itineraries.store') }}'" method="POST" class="p-6 space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="course_id" x-model="form.course_id" @change="onCourseChange()" required
                    ...
                    <div>
                        ...
                        <select name="module_id" x-model="form.module_id" required
                            ...
                            <template x-for="mod in filteredModules" :key="mod.id">
                                <option :value="mod.id" :selected="form.module_id == mod.id" x-text="mod.name"></option>
                            </template>
                        </select>
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">
                            Nombre del Itinerario / Tema <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="name" x-model="form.name" required maxlength="255"
                            placeholder="Ej: Sesión 1: Arquitectura básica y componentes de placa madre"
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        <button type="submit"
                            ...
                            <span x-text="isEdit ? 'Guardar Cambios' : 'Registrar Itinerario'"></span>
                        </button>
                    </div>
                </form>
            </div>
...
<script>
    ...
    function itinerariesApp() {
        return {
            ...
            onCourseChange() {
                const selected = coursesData.find(c => c.id == this.form.course_id);
                this.filteredModules = selected ? (selected.modules || []) : [];
            },
            ...
            openEditModal(itin) {
                this.isEdit = true;
                this.updateUrl = `{{ url('admin-itinerarios') }}/${itin.id}`;
                this.form = {
                    id: itin.id,
                    course_id: itin.course_id || '',
                    module_id: itin.module_id || '',
                    name: itin.name || '',
                    is_active: Boolean(itin.is_active),
                };
                this.onCourseChange();
                this.modalOpen = true;
            }
