@extends('layouts.app')
@section('title', 'Editar Procedimiento TUPA - Panel Administrativo')
@section('content')
        ...
        <header class="bg-white border-b border-gray-200 sticky top-[64px] lg:top-0 z-[30] shadow-sm backdrop-blur-md bg-white/90">
            <div class="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
                ...
                <div class="hidden sm:flex items-center text-sm font-medium text-gray-500">
                    ...
                    <span class="text-purple-600">Editar Procedimiento</span>
                </div>
            </div>
        </header>
            ...
            <div class="max-w-4xl mx-auto space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 space-y-8">
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r from-purple-500/10 via-indigo-500/10 to-blue-500/10 border border-purple-100">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-gray-900">Modificar Procedimiento: {{ $procedure->code }} - {{ $procedure->name }}</h3>
                            <p class="text-xs text-gray-500 mt-0.5">Actualiza la información del trámite, requisitos, costo o de la oficina que atiende.</p>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('admin.tupa.procedures.update', $procedure) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <select id="tupa_id" name="tupa_id" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('tupa_id') border-red-500 @enderror">
                                    ...
                                    @foreach($tupas as $tupa)
                                        <option value="{{ $tupa->id }}" {{ old('tupa_id', $procedure->tupa_id) == $tupa->id ? 'selected' : '' }}>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="code" name="code" value="{{ old('code', $procedure->code) }}" required class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('code') border-red-500 @enderror">
                                </div>
                                @error('code')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="name" name="name" value="{{ old('name', $procedure->name) }}" required class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('name') border-red-500 @enderror">
                                </div>
                                @error('name')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2 space-y-1.5">
                                ...
                                <textarea id="description" name="description" rows="3" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('description') border-red-500 @enderror">{{ old('description', $procedure->description) }}</textarea>
                                @error('description')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="md:col-span-2 space-y-1.5">
                                ...
                                @php
                                    $reqValue = old('requirements');
                                    if (is_null($reqValue)) {
                                        $reqValue = is_array($procedure->requirements) ? implode("\n", $procedure->requirements) : $procedure->requirements;
                                    } elseif (is_array($reqValue)) {
                                        $reqValue = implode("\n", $reqValue);
                                    }
                                @endphp
                                <textarea id="requirements" name="requirements" rows="4" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('requirements') border-red-500 @enderror">{{ $reqValue }}</textarea>
                                <p class="text-xs text-gray-400">Escriba cada requisito en una línea separada.</p>
                                @error('requirements')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="cost" name="cost" value="{{ old('cost', $procedure->cost) }}" required class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('cost') border-red-500 @enderror">
                                </div>
                                @error('cost')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="space-y-1.5">
                                ...
                                <select id="qualification" name="qualification" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('qualification') border-red-500 @enderror">
                                    <option value="Evaluación Previa (Positivo)" {{ old('qualification', $procedure->qualification) == 'Evaluación Previa (Positivo)' ? 'selected' : '' }}>Evaluación Previa (Positivo)</option>
                                    <option value="Evaluación Previa (Negativo)" {{ old('qualification', $procedure->qualification) == 'Evaluación Previa (Negativo)' ? 'selected' : '' }}>Evaluación Previa (Negativo)</option>
                                    <option value="Aprobación Automática" {{ old('qualification', $procedure->qualification) == 'Aprobación Automática' ? 'selected' : '' }}>Aprobación Automática</option>
                                </select>
                                @error('qualification')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="duration" name="duration" value="{{ old('duration', $procedure->duration) }}" required class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('duration') border-red-500 @enderror">
                                </div>
                                @error('duration')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2 space-y-1.5">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="office" name="office" value="{{ old('office', $procedure->office) }}" required class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('office') border-red-500 @enderror">
                                </div>
                                @error('office')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2 pt-2">
                                <label class="inline-flex items-center cursor-pointer gap-3 p-3 bg-gray-50 border border-gray-200 rounded-xl hover:bg-gray-100/80 transition-colors w-full">
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $procedure->is_active) ? 'checked' : '' }} class="sr-only peer">
                        ...
                        <div class="pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
                            ...
                            <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold text-sm rounded-xl shadow-md hover:from-purple-700 hover:to-indigo-700 transition-all duration-200 inline-flex items-center gap-2">
                                ...
                                <span>Actualizar Procedimiento</span>
                            </button>
                        </div>
                    </form>
