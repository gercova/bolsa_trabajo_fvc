@extends('layouts.app')
@section('title', 'Editar Categoría TUPA - Panel Administrativo')
@section('content')
    ...
    <div class="flex-1 flex flex-col min-w-0 bg-gray-50/50 relative">  
        <header class="bg-white border-b border-gray-200 sticky top-[64px] lg:top-0 z-[30] shadow-sm backdrop-blur-md bg-white/90">
            <div class="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
                ...
                <div class="hidden sm:flex items-center text-sm font-medium text-gray-500">
                    ...
                    <span class="text-purple-600">Editar Categoría</span>
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8 overflow-x-hidden">
            <div class="max-w-3xl mx-auto space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 space-y-8" x-data="{ selectedIcon: '{{ old('icon', $category->icon) }}' }">
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r from-purple-500/10 via-indigo-500/10 to-blue-500/10 border border-purple-100">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-gray-900">Modificar Categoría: {{ $category->name }}</h3>
                            <p class="text-xs text-gray-500 mt-0.5">Actualiza el nombre, ícono o visibilidad de la categoría.</p>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('admin.tupa.categories.update', $category) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}" required class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('name') border-red-500 @enderror">
                                </div>
                                @error('name')
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="relative flex-1">
                                        ...
                                        <input type="text" id="icon" name="icon" x-model="selectedIcon" value="{{ old('icon', $category->icon) }}" required class="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all @error('icon') border-red-500 @enderror">
                                    </div>
                                </div>
                                @error('icon')
                                    <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
