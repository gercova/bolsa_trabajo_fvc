...
@push('styles')
    <style>
        ...
        .paginator-btn {
            ...
            font-size: 0.8rem; font-weight: 600; border: 1.5px solid #e5e7eb;
            background: #ffffff; color: #374151; transition: all 0.15s;
        }
        .paginator-btn:hover:not(:disabled):not(.active) { border-color: #4f46e5; color: #4f46e5; background: #eef2ff; }
        .paginator-btn.active { background: #4f46e5; border-color: #4f46e5; color: #fff; font-weight: 800; }
        .paginator-btn:disabled { opacity: 0.4; cursor: not-allowed; }
        .type-pill-ordinaria   { background: #dbeafe; color: #1d4ed8; border: 1px solid #bfdbfe; }
        .type-pill-extraordinaria { background: #ede9fe; color: #6d28d9; border: 1px solid #ddd6fe; }
    </style>
@endpush
@section('content')
        ...
        <header class="bg-white border-b border-gray-200 sticky top-[64px] lg:top-0 z-[30] shadow-sm backdrop-blur-md bg-white/90">
            <div class="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
                <div class="flex items-center">
                    <button @click="toggleSidebar()" class="mr-3 sm:mr-4 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 p-2 rounded-lg transition-colors lg:hidden">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-5 py-4 border-b border-gray-100">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-sm font-extrabold text-gray-800 tracking-tight">Cronogramas Registrados</h2>
                                <p class="text-xs text-gray-500 mt-0.5">Gestiona los períodos de matrícula ordinaria y extraordinaria.</p>
                            </div>
                        </div>
                    ...
                    <form method="GET" action="{{ route('admin.enrollments.index') }}"
                        ...
                        <div class="relative flex-1">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar período u observaciones..."
                        ...
                        <select name="type" class="text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 bg-slate-50">
                            <option value="">Todos los tipos</option>
                            <option value="ordinaria" {{ request('type') === 'ordinaria' ? 'selected' : '' }}>Ordinaria</option>
                            <option value="extraordinaria" {{ request('type') === 'extraordinaria' ? 'selected' : '' }}>Extraordinaria</option>
                        </select>
                        <select name="status" class="text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 bg-slate-50">
                            <option value="">Todos los estados</option>
                            <option value="active" {{ request('status') === 'active' ? 'selected' : '' }}>Activos</option>
                            <option value="inactive" {{ request('status') === 'inactive' ? 'selected' : '' }}>Inactivos</option>
                        </select>
                        <button type="submit" class="bg-indigo-600 text-white text-sm font-bold px-4 py-2 rounded-xl hover:bg-indigo-700 transition">
                            <i class="bi bi-funnel mr-1"></i> Filtrar
                        </button>
                        @if (request()->hasAny(['search', 'type', 'status']))
                            <a href="{{ route('admin.enrollments.index') }}" class="text-sm text-gray-500 hover:text-red-500 flex items-center gap-1 px-3 py-2 rounded-xl border border-gray-200 hover:border-red-200 transition">
                    ...
                    <div class="overflow-x-auto custom-scrollbar">
                        <table class="w-full text-sm min-w-[700px]">
                            <thead class="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    <th class="px-5 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">#</th>
                                    <th class="px-5 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Período / Tipo</th>
                                    <th class="px-5 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Inicio</th>
                                    <th class="px-5 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Cierre</th>
                                    <th class="px-5 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Costo (S/)</th>
                                    <th class="px-5 py-3 text-left text-xs font-extrabold text-gray-500 uppercase tracking-wider">Estado</th>
                                    <th class="px-5 py-3 text-right text-xs font-extrabold text-gray-500 uppercase tracking-wider">Acciones</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50">
                                @forelse ($schedules as $schedule)
                                    <tr class="hover:bg-indigo-50/30 transition-colors">
                                        ...
                                        <td class="px-5 py-4">
                                            <div class="font-extrabold text-gray-800">{{ $schedule->academic_period }}</div>
                                            <span class="mt-1 inline-flex items-center text-xs font-bold px-2.5 py-0.5 rounded-full
                                        ...
                                        <td class="px-5 py-4">
                                            <form action="{{ route('admin.enrollments.toggle-status', $schedule) }}" method="POST" class="inline">
                                                @csrf @method('PATCH')
                                                <button type="submit" class="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full transition
                                                    {{ $schedule->is_active
                                                        ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'
                                                        : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }}">
                                                    <span class="w-1.5 h-1.5 rounded-full {{ $schedule->is_active ? 'bg-emerald-500' : 'bg-gray-400' }}"></span>
                                    ...
                                    <tr>
                                        <td colspan="7" class="px-5 py-16 text-center">
                                            <div class="flex flex-col items-center gap-3 text-gray-400">
                                                ...
                                                <p class="font-semibold text-base text-gray-500">No se encontraron cronogramas de matrícula.</p>
                                                <a href="{{ route('admin.enrollments.create') }}"
                    ...
                    @if ($schedules->hasPages())
                        <div class="px-5 py-4 border-t border-gray-100 flex items-center justify-between gap-4">
                            <p class="text-xs text-gray-500">
                                Mostrando <strong>{{ $schedules->firstItem() }}</strong>–<strong>{{ $schedules->lastItem() }}</strong>
                                de <strong>{{ $schedules->total() }}</strong> cronogramas
                            </p>
                            <div class="flex items-center gap-1.5 flex-wrap">
                                <a href="{{ $schedules->previousPageUrl() }}" class="paginator-btn {{ $schedules->onFirstPage() ? 'opacity-30 pointer-events-none' : '' }}">
                                    <i class="bi bi-chevron-left text-xs"></i>
                                </a>
                                @foreach ($schedules->getUrlRange(1, $schedules->lastPage()) as $page => $url)
                                    <a href="{{ $url }}" class="paginator-btn {{ $schedules->currentPage() === $page ? 'active' : '' }}">{{ $page }}</a>
                                @endforeach
                                <a href="{{ $schedules->nextPageUrl() }}" class="paginator-btn {{ !$schedules->hasMorePages() ? 'opacity-30 pointer-events-none' : '' }}">
