...
@push('styles')
    ...
    <meta name="keywords"
        content="plana docente, profesores iestp, coordinadores academicos, educadores tecnicos, francisco vigo caballero, uchiza, docentes produccion agropecuaria, docentes enfermeria tecnica, docentes redes, docentes administracion, docentes manejo forestal, docentes transversales">
    ...
    @php
        $transversalList = $transversalTeachers ?? $unassignedTeachers ?? collect();
        $schemaItemList = [];
        $pos = 1;

        // Add details from assigned teachers
        ...
        // Add transversal teachers to Schema
        foreach ($transversalList as $tTeacher) {
            $schemaItemList[] = [
                '@type' => 'ListItem',
                'position' => $pos++,
                'item' => [
                    '@type' => 'Person',
                    'name' => $tTeacher->names,
                    'jobTitle' => $tTeacher->job_position ?? 'Docente de Formación Transversal',
                    'email' => $tTeacher->email ?? null,
                    'worksFor' => [
                        '@type' => 'EducationalOrganization',
                        'name' => 'IESTP Francisco Vigo Caballero',
                    ],
                    'knowsAbout' => 'Formación General y Módulos Transversales',
                ]
            ];
        }
    @endphp
...
@section('content')
    @php
        ...
        $totalTeachersCount = $teacherDetails
            ->pluck('user_id')
            ->concat($transversalList->pluck('id'))
            ->unique()
            ->count();
    ...
    <div x-data="{
        ...
        openTeacherModal(name, role, program, specialty, email, photo, isCoordinator, cvUrl) {
            this.activeTeacher = {
                name: name,
        ...
        <section class="relative bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 text-white overflow-hidden py-16 lg:py-24 border-b border-blue-900/30">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center space-y-8">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-5xl mx-auto mt-16 pt-12 border-t border-white/10">
                    ...
                    <div class="bg-white/5 backdrop-blur-md p-6 rounded-2xl border border-white/5 hover:border-sky-500/20 transition-all duration-300">
                        <p class="text-3xl sm:text-4xl font-black text-indigo-400">{{ $transversalList->count() }}</p>
                        <p class="text-sm sm:text-base font-bold text-slate-400 mt-2">Docentes Transversales</p>
                    </div>
                </div>
            </div>
        </section>
                                ...
                                <div class="coordinator-card bg-white rounded-3xl border border-amber-200/90 shadow-md hover-card p-6 flex flex-col justify-between relative overflow-hidden group cursor-pointer"
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center gap-2 mb-4 flex-wrap">
                                            ...
                                            @if ($user->cv_file)
                                                <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-red-50 text-red-700 border border-red-200 text-[11px] font-bold" title="CV en PDF disponible">
        ...
        {{-- ===== INTERACTIVE PROGRAM FILTER & SEARCH BAR ===== --}}
        <section class="sticky top-[var(--nav-h,72px)] z-20 bg-white/95 backdrop-blur-md border-b border-slate-200/90 py-4 shadow-sm">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-none">
                    <button type="button" @click="selectedProgram = 'all'"
                        class="px-4 py-2 rounded-xl text-xs sm:text-sm font-black transition-all shrink-0 cursor-pointer flex items-center gap-1.5 shadow-sm"
                        :class="selectedProgram === 'all'
                            ? 'bg-slate-900 text-white shadow-slate-900/20'
                            : 'bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200'">
                        <i class="bi bi-grid-fill"></i>
                        <span>Todos</span>
                        <span class="ml-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold"
                            :class="selectedProgram === 'all' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-700'">
                            {{ $totalTeachersCount }}
                        </span>
                    </button>
                    ...
                    @foreach ($programs as $p)
                        @php
                            $pCount = $teacherDetails->where('program_id', $p->id)->where('is_coordinator', false)->count();
                            $pIcon = $programMeta[$p->name]['icon'] ?? 'bi-mortarboard-fill';
                        @endphp
                        <button type="button" @click="selectedProgram = '{{ $p->id }}'"
                            class="px-3.5 py-2 rounded-xl text-xs font-bold transition-all shrink-0 cursor-pointer flex items-center gap-1.5"
                            :class="selectedProgram === '{{ $p->id }}'
                                ? 'bg-blue-600 text-white shadow-md shadow-blue-600/25'
                                : 'bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200'">
                            <i class="bi {{ $pIcon }}"></i>
                            <span class="hidden sm:inline">{{ $p->name }}</span>
                            <span class="sm:hidden">{{ Str::limit($p->name, 16) }}</span>
                            <span class="px-1.5 py-0.5 rounded-full text-[10px]"
                                :class="selectedProgram === '{{ $p->id }}' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-600'">
                                {{ $pCount }}
                            </span>
                        </button>
                    @endforeach

                    {{-- Transversales Filter Tab --}}
                    @if ($transversalList->isNotEmpty())
                        <button type="button" @click="selectedProgram = 'transversales'"
                            class="px-3.5 py-2 rounded-xl text-xs font-bold transition-all shrink-0 cursor-pointer flex items-center gap-1.5"
                            :class="selectedProgram === 'transversales'
                                ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/25'
                                : 'bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200'">
                            <i class="bi bi-diagram-3-fill"></i>
                            <span>Transversales</span>
                            <span class="px-1.5 py-0.5 rounded-full text-[10px]"
                                :class="selectedProgram === 'transversales' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-600'">
                                {{ $transversalList->count() }}
                            </span>
                        </button>
                    @endif
                </div>
                ...
                <div class="relative w-full md:w-72 shrink-0">
                    ...
                    <input type="text" x-model="searchQuery"
                        ...
                        class="w-full pl-9 pr-8 py-2 bg-slate-100 border border-slate-200 rounded-xl text-xs font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all">
        ...
        <section class="py-20 bg-slate-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
                ...
                @foreach ($programs as $program)
                    @php
                        // Exclude coordinators from each study program's teacher list
                        $progTeachers = $teacherDetails
                    ...
                    <div x-show="(selectedProgram === 'all' || selectedProgram === '{{ $program->id }}')"
                        ...
                        {{-- Teachers Cards Grid (Excluding Coordinators) --}}
                                ...
                                @foreach ($progTeachers as $detail)
                                    ...
                                    @if ($user)
                                        <div x-show="matchesSearch('{{ addslashes($user->names) }}', '{{ addslashes($detail->specialty ?? '') }}', '{{ addslashes($user->job_position ?? '') }}', '{{ addslashes($program->name) }}')"
                                             ...
                                             @click="openTeacherModal('{{ addslashes($user->names) }}', '{{ addslashes($user->job_position ?? 'Docente') }}', '{{ addslashes($program->name) }}', '{{ addslashes($detail->specialty ?? 'Especialista') }}', '{{ addslashes($user->email ?? '') }}', '{{ $user->photo_profile ? Storage::url($user->photo_profile) : '' }}', false, '{{ $user->cv_file ? Storage::url($user->cv_file) : '' }}')">
                                            ...
                                            <div class="pt-4 border-t border-slate-100 flex items-center justify-between text-xs mt-2">
                                                @if ($user->email)
                                                    <a href="mailto:{{ $user->email }}" @click.stop title="{{ $user->email }}"
                                                        class="inline-flex items-center gap-1.5 font-bold text-slate-500 hover:text-blue-600 transition-colors truncate max-w-[70%]">
                                                ...
                                                <div class="flex items-center gap-1.5 shrink-0">
                                                    @if ($user->cv_file)
                                                        <span class="text-red-500 text-sm" title="CV en PDF disponible">
                                                            <i class="bi bi-file-earmark-pdf-fill"></i>
                                                        </span>
                                                    @endif
                                                    <span class="w-2.5 h-2.5 rounded-full bg-emerald-500" title="Docente Activo"></span>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                ...
                {{-- ===== SECTION: TRANSVERSALES (Teachers without an assigned program) ===== --}}
                @if ($transversalList->isNotEmpty())
                    <div x-show="(selectedProgram === 'all' || selectedProgram === 'transversales')"
                        x-transition:enter="transition ease-out duration-300"
                        x-transition:enter-start="opacity-0 translate-y-4"
                        x-transition:enter-end="opacity-100 translate-y-0"
                        id="seccion-transversales"
                        class="space-y-8 pt-8 border-t border-slate-200">
                        ...
                        <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-md flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <span class="px-3 py-1 text-xs font-black rounded-lg uppercase tracking-wider border mb-1 inline-block bg-indigo-50 text-indigo-800 border-indigo-100">
                                        Formación General & Módulos Comunes
                                    </span>
                                    <h2 class="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
                                        Transversales
                                    </h2>
                            ...
                            <div class="flex items-center gap-3">
                                <span class="px-4 py-2 rounded-xl bg-indigo-50 text-indigo-800 text-xs font-extrabold border border-indigo-100">
                                    {{ $transversalList->count() }} Docentes Transversales
                                </span>
                            </div>
                        </div>

                        {{-- Transversales Cards Grid --}}
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                            @foreach ($transversalList as $teacher)
                                @php
                                    $specialtyText = $teacher->primaryRoleDetail?->specialty ?? 'Módulos Transversales y Empleabilidad';
                                @endphp
                                <div x-show="matchesSearch('{{ addslashes($teacher->names) }}', '{{ addslashes($specialtyText) }}', '{{ addslashes($teacher->job_position ?? '') }}', 'Transversales')"
                                    class="group bg-white rounded-3xl border border-indigo-100/80 shadow-md hover-card overflow-hidden flex flex-col justify-between h-full p-6 cursor-pointer relative"
                                    @click="openTeacherModal('{{ addslashes($teacher->names) }}', '{{ addslashes($teacher->job_position ?? 'Docente de Formación Transversal') }}', 'Formación General y Transversal', '{{ addslashes($specialtyText) }}', '{{ addslashes($teacher->email ?? '') }}', '{{ $teacher->photo_profile ? Storage::url($teacher->photo_profile) : '' }}', false, '{{ $teacher->cv_file ? Storage::url($teacher->cv_file) : '' }}')">
                                    ...
                                    <div>
                                        <div class="flex items-center gap-4 mb-4">
                                            ...
                                            <div class="min-w-0 flex-1 pr-2">
                                                <h3 class="text-base font-black text-slate-900 group-hover:text-indigo-600 transition-colors leading-snug truncate">
                                                    {{ $teacher->names }}
                                                </h3>
                                                <p class="text-xs font-bold text-slate-500 truncate mt-0.5">
                                                    {{ $teacher->job_position ?? 'Docente Transversal' }}
                                                </p>
                                            </div>
                                        </div>

                                        {{-- Tag / Badge --}}
                                        <div class="bg-indigo-50/60 p-3.5 rounded-xl border border-indigo-100/80 mb-4">
                                            <p class="text-[10px] font-black text-indigo-900 uppercase tracking-wider mb-0.5">
                                                Área de Formación
                                            </p>
                                            <p class="text-xs font-bold text-slate-700 leading-snug line-clamp-2">
                                                {{ $specialtyText }}
                                    ...
                                    {{-- Footer Email & CV Indicator --}}
                                    <div class="pt-4 border-t border-slate-100 flex items-center justify-between text-xs mt-2">
                                        @if ($teacher->email)
                                            <a href="mailto:{{ $teacher->email }}" @click.stop title="{{ $teacher->email }}"
                                                class="inline-flex items-center gap-1.5 font-bold text-slate-500 hover:text-indigo-600 transition-colors truncate max-w-[70%]">
                                                <i class="bi bi-envelope text-indigo-600"></i>
                                                <span class="truncate">{{ $teacher->email }}</span>
                                            </a>
                                        @else
                                            <span class="text-slate-400 italic text-[11px]">Docente Institucional</span>
                                        @endif
        ...
        {{-- ===== TEACHER PROFILE DETAIL MODAL WITH CV (PDF) (Alpine.js) ===== --}}
        <div x-show="showModal"
            ...
            <div x-show="showModal"
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-wider text-slate-400">Programa de Estudio / Área</p>
                    ...
                    <div>
                        ...
                        <div class="bg-slate-50 p-3.5 rounded-xl border border-slate-200 mt-1">
                    ...
                    {{-- CV (PDF) Section --}}
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1.5">
                            Currículum Vítae (CV)
                        </p>
                            ...
                            <a :href="activeTeacher.cvUrl" target="_blank" rel="noopener noreferrer"
                               class="flex items-center justify-between p-3.5 rounded-2xl bg-gradient-to-r from-red-500/10 via-rose-500/10 to-blue-500/10 border border-red-200/80 hover:border-red-400 hover:shadow-md transition-all group/cv">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="text-left">
                                        <p class="text-xs font-black text-slate-900 group-hover/cv:text-red-600 transition-colors">
                                            Descargar / Ver CV en PDF
                                        </p>
                                        <p class="text-[11px] text-slate-500 font-medium">
                                            Documento oficial de trayectoria profesional
                                        </p>
                                    </div>
                                </div>
                                <span class="inline-flex items-center gap-1 text-xs font-bold text-red-600 px-3 py-1.5 rounded-lg bg-white border border-red-200 shadow-sm shrink-0 group-hover/cv:bg-red-600 group-hover/cv:text-white transition-colors">
                                    <span>Abrir</span>
                        ...
                        <template x-if="!activeTeacher || !activeTeacher.cvUrl">
                            <div class="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-500 text-xs font-medium">
                                <div class="w-8 h-8 rounded-lg bg-slate-200 text-slate-400 flex items-center justify-center text-sm shrink-0">
                                    <i class="bi bi-file-earmark-text"></i>
                                </div>
                                <span>Currículum Vítae en proceso de actualización institucional</span>
                            </div>
                        </template>
                    </div>
